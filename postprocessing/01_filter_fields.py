...
# which stays lazy on this chunked input). Writing a lazy Dataset via .to_netcdf() normally computes it via
# dask's default *threaded* scheduler during the write, with multiple threads writing into the same HDF5
# file handle -- a known hang risk, since the underlying HDF5 C library isn't reliably thread-safe for it
# (see local_potential_energies_timeseries() in aux01_pe_functions.py for a real observed stall of this
# kind). Forcing the *synchronous* (single-threaded) scheduler for just this write removes the concurrent-
# thread hazard while still streaming/computing chunk-by-chunk -- unlike an eager .load() first, this
# doesn't require the whole dataset to fit in memory at once (an earlier version of this fix used .load()
# and OOM-killed a production-scale HPC run).
