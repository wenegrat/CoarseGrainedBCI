...
for ℓ in filter_scales:
    ...
    # mixed Dataset via .to_netcdf() normally computes the lazy pieces via dask's default *threaded*
    # scheduler during the write, with multiple threads writing into the same HDF5 file handle. Forcing the
    # *synchronous* (single-threaded) scheduler for just this write removes the concurrent-thread hazard
    # without requiring budget_ℓ to be fully materialized in memory first (an earlier .load()-based version
    # of this fix OOM-killed a production-scale HPC run).
    print(f"  Saving checkpoint (single-threaded scheduler, to avoid concurrent-thread HDF5 writes)...")
    t0 = time.time()
...
# ds_full.ρ. Same hang risk as the checkpoint write above; the synchronous scheduler avoids it without
# requiring the whole (multi-scale) dataset to fit in memory at once.
