...
parser = argparse.ArgumentParser(description="Depth profile of horizontally- and time-averaged Πₖ, Π_A, "
                                              "and buoyancy conversion (SFS APE->KE exchange) at one filter scale")
...
#+++ Load Πₖ/conversion (ke_fields) and Π_A (ape_fields); pick filter scale, dedup + pool over the
...
# default window can't even start, and silently plotting from whatever scraps exist near the run's actual
...
print(f"ℓ = {ℓ:.4f} m ({ℓ_km} km), t = {t_min_days:.2f}-{t_max_days:.2f} days ({n_times} snapshots)")
...
# Matched to ke_t's own (already deduped/windowed) times independently -- the two files can have slightly
# different time grids/roundoff, same reasoning as plot5_vorticity_strain_flux.py's own Π_A selection.
...
# regardless of Δt, matching sweep3/plot5's own convention for multi-snapshot pooling.
...
profiles = {
    "Πₖ":                  (Pi_K_hz.mean("time"), Pi_K_hz.std("time"), "#2166ac"),
    "Π_A":                 (Pi_A_hz.mean("time"), Pi_A_hz.std("time"), "#d6604d"),
    "buoyancy conversion": (conv_hz.mean("time"), conv_hz.std("time"), "#1b7837"),
}
#---

#+++ Plot: single panel, depth on y, value on x -- one line (+/-1 std shaded band across the pooled
# snapshots) per quantity, all sharing one m² s⁻³ axis since they're the same physical units (unlike the
# map panels elsewhere in this pipeline, a shared line-plot x-axis doesn't wash out a weaker term the way
# a shared color scale would -- each line's own shape stays visible regardless of the others' magnitude).
print("Building figure...")
fig, ax = plt.subplots(figsize=(7, 8), constrained_layout=True)

for name, (mean, std, color) in profiles.items():
...
title = f"{stem}, ℓ={ℓ_km}km, t={t_min_days:.1f}-{t_max_days:.1f}d ({n_times} snapshots)"
if label:
    title = f"{label}\n{title}"
ax.set_title(title, fontsize=11)

outfile = FIGURES / f"{stem}_depth_profile_l{ℓ_km}km_t{t_min_days:.0f}-{t_max_days:.0f}d.pdf"
