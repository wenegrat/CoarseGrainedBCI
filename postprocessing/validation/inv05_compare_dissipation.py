...
from src.aux00_utils import load_dataset_and_grid, make_gaussian_filter, condense_uw_velocities, integrate
...
#+++ Configuration
import argparse
parser = argparse.ArgumentParser(description="Compare online (simulation-time) vs offline (post-processed) SFS KE dissipation ε_Kˢ")
parser.add_argument("--filename", default="output/khi_Nz256_Ri0.10.nc", help="Path to simulation NetCDF file (run with Oceanostics PR #259)")
...
def online_name(ℓ):
    """Online output variable name for filter scale ℓ (matches the Julia Symbol("ε_Ks_ℓ$(ℓ)"))."""
    return f"ε_Ks_ℓ{int(ℓ)}" if ℓ == int(ℓ) else f"ε_Ks_ℓ{ℓ}"
#---

#+++ Load dataset
# load_dataset_and_grid pads the z domain to 2× (edge values); this pads *both* the online ε_Kˢ
# variables read from the file and the fields used to recompute ε_Kˢ offline, so the two live on the
# same grid. We recover the original (unpadded) z extent from the grid group to drop the padding
# before any comparison (the padded region is ≈0 for ε_Kˢ and would otherwise dilute the metrics).
...
print("Recomputing ε_Kˢ offline at each filter scale...")
offline = {}
for ℓ in args.filter_scales:
    gf = make_gaussian_filter(ℓ, ds)
    offline[ℓ] = calculate_sfs_ke_dissipation(S, ν, gf, filter_dims=filtered_dimensions).rename(online_name(ℓ))
    print(f"  ℓ = {ℓ:>4}: offline ε_Kˢ computed")
#---

#+++ Load the online ε_Kˢ fields / integrals
online = {}
online_int = {}
for ℓ in args.filter_scales:
    name = online_name(ℓ)
...
if not filter_scales:
    raise SystemExit("No online ε_Kˢ fields found — run the simulation (Oceanostics PR #259) first.")
#---

#+++ Snapshot maps: online | offline | difference  (one row per filter scale; ε_Kˢ ≥ 0)
